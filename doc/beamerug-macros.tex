...
\item{\InlineClass{classoption-heading}{{\ttfamily\char`\\documentclass[\declare{#1}]\char`\{beamer\char`\}}}}
...
\item \InlineClass{element-headline}{\textbf{\ifx#2\semiyes Parent Beamer-Template\else%
    Beamer\applier#2{-Template}\applier#3{\applier#2{/}-Color}\applier#4{\ifx#2\yes/\else\ifx#3\yes/\fi\fi
      -Font}\fi}
    {\ttfamily{\InlineClass{element-declare}{\declare{#1}}}}}\par\topsep=0pt%
  \edef\parameters{%
...
\newcommand\translatormanualentryheadline[1]{%
  \BlockClass{translatormanualentryheadline}{\itemsep=0pt\parskip=0pt\item\strut#1\par\topsep=0pt}\endBlockClass}
